...
namespace BrokemiaHelper {
    ...
    public class NonBadelineMovingBlock : Solid {
        ...
        public NonBadelineMovingBlock(Vector2[] nodes, float width, float height, char tiletype, char highlightTiletype, string flag, float delay, float travelTime)
            ...
            sprite = GFX.FGAutotiler.GenerateBox(tiletype, (int) Width / 8, (int) Height / 8).TileGrid;
            Add(sprite);
            Calc.PopRandom();
            Calc.PushRandom(newSeed);
            highlight = GFX.FGAutotiler.GenerateBox(highlightTiletype, (int) (Width / 8f), (int) Height / 8).TileGrid;
            highlight.Alpha = 0f;
            Add(highlight);
            Calc.PopRandom();
        ...
        public override void Update() {
            ...
            if(moveCoroutine == null && (string.IsNullOrWhiteSpace(startFlag) || SceneAs<Level>().Session.GetFlag(startFlag))) {
                StartMoving();
            }
        }
