...
namespace BrokemiaHelper {
    [CustomEntity("BrokemiaHelper/persistentMiniTextboxTrigger")]
    public class PersistentMiniTextboxTrigger : Trigger {
		...
		private void Trigger() {
			...
			if (!triggered && (deathCount < 0 || SceneAs<Level>().Session.DeathsInCurrentLevel == deathCount)) {
				triggered = true;
				Scene.Add(new PersistentMiniTextbox(Calc.Random.Choose(dialogOptions), time: time));
				if (onlyOnce) {
